...
export class UserController {
  ...
  public all = catchError(async (request: Request, response: Response, next: NextFunction) => {
    const users = await this.Users.getAll();
    response.status(200).send(users);
  });

  public one = catchError(async (request: Request, response: Response, next: NextFunction) => {
  ...
  public save = catchError(async (request: Request, response: Response, next: NextFunction) => {
  ...
  public remove = catchError(async (request: Request, response: Response, next: NextFunction) => {
    const userId = request.params.id;
    if (!userId) throw new BadRequestError();
